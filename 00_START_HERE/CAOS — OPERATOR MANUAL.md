...
**Current state of this vault:** the one-time intake below is complete and the Master Ledger has been `INITIALIZED` since 2026-08-31, reconciled against a Revolut broker export (8 inherited holdings, real cash stamped in Master Ledger §2–§3). Portfolio-aware products are no longer forced into research-only/degraded mode by default — each run still checks the Ledger's live status per §4 below and degrades if it goes stale or is otherwise unreadable. The 8 inherited holdings are marked `CORE/ATTACKER` provisionally pending formal role assignment in the first post-intake Deep Audit (Master Ledger §4) — do not treat that provisional label as a completed thesis review.

Only **Daily Anchor** has actually been run end-to-end through the real Agent-tool pipeline (2026-08-29, before the intake — see its dated files in `03_AGENT_RUNS/`, all `UNINITIALIZED` at that time, correctly). **Post-Open Delta Check** is labeled `ACTIVE` on the Command Card but has no dated output file anywhere in this vault — treat that label as aspirational, not evidence, until it has actually run once. **Weekly Ranking** and **Monster Census** have never been executed through the Agent tool either; the narrative walkthrough files that once sat in `03_AGENT_RUNS/09_ORCHESTRATOR/` claiming otherwise have been moved to `04_FLIGHT_RECORDER/` and marked non-canonical — their required upstream specialist files never existed, and one contained a portfolio NAV figure that was wrong by roughly 430x. Do not treat any file's presence in a `03_AGENT_RUNS/0X_ROLE/` folder as proof a real run happened — verify its upstream "Inputs Consulted" links actually resolve to files that exist.
...
See [[00_START_HERE/CAOS — COMMAND CARD]] for the full command list and per-product status. As of 2026-09-02, all 10 products have a written runbook — but "written" and "proven" are different things:

- **Daily Anchor** is the only product with a real, verified end-to-end run behind it (2026-08-29).
- **Post-Open Delta Check, Emergency Thesis Rerun, DCA Execution Card, Deep Audit, Learning Review, Event Gate Watch, and Multi-Bagger Hunter Watch** are built (full runbooks, reusing the 9-agent roster where applicable) but have never been executed for real.
- **Weekly Ranking and Monster Census** are built, and the only prior "run" evidence for either was a set of fabricated narrative walkthroughs — never produced by the Agent tool — now quarantined in `04_FLIGHT_RECORDER/` with a `NON-CANONICAL` banner. Treat them as unproven until a real run happens.

Do not treat a runbook's existence, or a file sitting in `03_AGENT_RUNS/`, as proof a product works — verify its "Inputs Consulted" links actually resolve to files that exist.
...
- Typing any of the other 7 built-but-unproven Command Card commands (Emergency Thesis Rerun, DCA Execution Card, Monster Census, Weekly Ranking, Deep Audit, Learning Review, Event Gate Watch, Multi-Bagger Hunter Watch) follows that product's own runbook precondition checks and produces, at minimum, a stated `BLOCKED`/`DEGRADED`/`INCOMPLETE` result naming exactly what ran and what didn't — never a silent failure and never an improvised result that skips the runbook's agent call sequence.
